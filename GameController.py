		...
		if (self.gameState == 0):
			...
			if (updateInput.lower() == 'done'):
				...
				if (response == 10):
					...
					self.chatFrame.handleReceiveMessage("You failed to save howdy hack. You are fired! (YOU LOSE)", "Boss")
			...
			if (updateInput.lower() == 'help'):
				self.chatFrame.handleReceiveMessage('Take a look at this: ' + str(self.sqlChallenge.getResource()), "Boss")
			else:
				...
				if (response):
					...
					self.chatFrame.handleReceiveMessage('I can notify the authorities to prevent ' + self.sqlChallenge.getKeyInfo() + ' from hacking us ever again!', "Boss")
